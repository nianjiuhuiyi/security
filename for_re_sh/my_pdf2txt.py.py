...
def a_folder(q, pdf_dir, save_path):
    """
    针对一个文件夹(即一个上市公司)的处理
    @param q: 用于多进程通讯的队列
...
if __name__ == '__main__':
    ...
    这是第三步：把pdf转成txt
    """
    txt_save_path = r"C:\Users\Administrator\Desktop\SH_2\temp_txt"
    total_path = r"C:\Users\Administrator\Desktop\SH_2\temp_data"  # pdf总路径
    ...
    for com_code_name in name_files:
        ...
        pool.apply_async(a_folder, args=(queue, pdf_dir, save_path))
    pool.close()   # 还是关闭进程池，但是不需要下面那句等待了
    # pool.join()
    print("开始处理：")
    nums = 0
    while True:
        flag = queue.get()
        nums += 1
        print(colored("\r处理进度：{:0.2%}".format(nums / len(name_files)), "red"), end="")
        if nums == len(name_files):
            break
    print("\nAll is done!")

    # linux(如果用的话，要改，还没加进程间的通讯)
